...
function init(){
				...
				{   "aProcessing": true,
		        	...
		        	"aoColumns":[
		        	...
		        	],"ajax":
			        	{
							...
							type : "get",
							dataType : "json",
							data:{fecha_desde: fecha_desde, fecha_hasta: fecha_hasta, idsucursal: 1-2},
							error: function(e){
						   		console.log(e.responseText);
							}
			        	},
	...
	function ListadoVentasAnuladas(){
		...
		if($("#cboFechaDesdeAnuVent").val() != "" && $("#cboFechaHastaAnuVent").val() != ""){
			var fecha_desde = $("#cboFechaDesdeAnuVent").val(), fecha_hasta = $("#cboFechaHastaAnuVent").val(), idsucursal = $("#txtIdSucursal").val();
			var tabla = $('#tblVentaAnuladas').dataTable(
				{   "aProcessing": true,
		       		"aServerSide": true,
	       			dom: 'Bfrtip',
			        buttons: [
