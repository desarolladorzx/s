  ...
  for (var pos in data) {
    $("table#tblDetalleDoc").append(
      "<tr><td>" +
        ...
        "</td><td>" +
        data[pos][4] +
        "</td><td><button type='button' onclick='eliminarDetalleDocSuc(" +
        pos +
...
function cargarDataDocSucursal(
...
function eliminarDocSucursal(idDocSucursal) {
  bootbox.confirm("¿Esta Seguro de Eliminar el registro?", function (result) {
    if (result) {
      $.post(
        "./ajax/DocSucursalAjax.php?op=delete",
        { idDocSucursal: idDocSucursal },
        function (e) {
