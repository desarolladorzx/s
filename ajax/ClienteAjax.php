	...
	switch ($_GET["op"]) {
		...
		case 'SaveOrUpdate':			
			...
			if(empty($_POST["txtIdPersona"])){
				if($objCliente->Registrar($tipo_persona,$nombre,$apellido,$tipo_documento,$num_documento,$direccion_departamento,$direccion_provincia,$direccion_distrito,$direccion_calle,$telefono,$email,$numero_cuenta,$estado)){
				...
				if($objCliente->Modificar($idpersona,$tipo_persona,$nombre,$apellido,$tipo_documento,$num_documento,$direccion_departamento,$direccion_provincia,$direccion_distrito,$direccion_calle,$telefono,$email,$numero_cuenta,$estado)){
            ...
            $i = 1;
     		while ($reg = $query_Tipo->fetch_object()) {
     			$data[] = array(
					...
					"id"=>$i,
					"1"=>$reg->numero_cuenta.'&nbsp;'.$reg->tipo_persona,
