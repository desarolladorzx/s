		...
		case "list":
			...
			while ($reg = $query_Pedido->fetch_object()) {
				...
				$data[] = array("0"=>$i,
					...
					"7"=>($reg->estado=="A")?'<button class="btn btn-success" data-toggle="tooltip" title="Ver Detalle" onclick="cargarDataPedido('.$reg->idpedido.',\''.$reg->tipo_pedido.'\',\''.$reg->numero.'\',\''.$reg->APCliente.'\',\''.$reg_total->Total.'\',\''.$reg->email.'\',\''.$reg->direccion_calle.'\',\''.$reg->num_documento.'\',\''.$reg->telefono.'\',\''.$reg->fecha.'\')" ><i class="fa fa-eye"></i> </button>&nbsp'.
					...
					'<button class="btn btn-success" data-toggle="tooltip" title="Ver Detalle" onclick="cargarDataPedido('.$reg->idpedido.',\''.$reg->tipo_pedido.'\',\''.$reg->numero.'\',\''.$reg->APCliente.'\',\''.$reg_total->Total.'\',\''.$reg->direccion_calle.'\',\''.$reg->num_documento.'\',\''.$reg->telefono.'\',\''.$reg->fecha.'\')" ><i class="fa fa-eye"></i> </button>&nbsp'.
					'<a href="./Reportes/exTicket.php?id='.$reg->idpedido.'" class="btn btn-primary" data-toggle="tooltip" title="Imprimir" target="blanck" ><i class="fa fa-file-text"></i> </a>&nbsp;');
				$i++;
			}
				$results = array(
